...
def add_a_snak(cur,id,obj,localId):
	...
	value = [id,datatype,datavalue_type,datavalue_value,pproperty,snaktype,str(localId)]
	cur.execute('replace into snaks values(%s,%s,%s,%s,%s,%s,%s)', value)
			...
			try:
				...
				for (k,v) in decodejson['claims'].items():
					for i in v:
						...
						claim_type = i['type']
						claim_rank = i['rank']
						add_a_snak(cur,id,i['mainsnak'],claim_id)
						...
						if (i.get('references',None)!=None):
							for sub_obj in i['references']:
								for (kk,vv) in sub_obj['snaks'].items():
									...
									add_a_snak(cur,id,vv[0],claim_id)
